# 황금새장을열다: Opening the Golden Cage

책 홈페이지: https://hyunjimoon.github.io/golden-cage-book/

> 창업자의 네 가지 빚과 네 가지 열쇠

**저자**: 문현지 (Angie Hyunji Moon)
**출판사**: 민음사 (검토 중)
**근거 논문**: [Golden Cage: Paradox of Early Funding and Growth](https://github.com/hyunjimoon/golden-cage-thesis) (MIT SM Thesis, 2026)

---
## 6*4=24개 상징의 길 위에서 4명의 인물들과 함께 변신하는 체험서 

💃무도회장의 엠마는 🪞거울과 💸뢰르를 🤨의심하며, ?처럼 📒장부를 ✍️써라

⚓️항구의 살리나는 🔭망원경과 🥀꽃을 🤨의심하며, 탄크레디처럼 🎞️순서를 🏄‍♀️타라   

🏙️도시의 파눌루는 💒스테인글라스와 🦠병독을 🤨의심하며, 리유와 타루처럼 🧠믿음을 🔄교정하라  

🏝️섬의 로빈슨은 ⏰시계와 🌾곡식창고를 🤨의심하며, 방드르디처럼 🔥불을 🚬피워라 


## 핵심 방정식

```
dG/dF = (dG/dR) × (dR/dF) = (+) × (−) = (−)
```

자원(F)이 방향전환(R)을 억압하고, 방향전환이 성장(G)을 만든다. 168,011개 미국 벤처 실증.

## 구조: 네 소설, 네 새장, 네 약

| 장 | 새장 | 소설 | 빚 | 여는 법 | 산업 | AI 도구 |
|:--|:--|:--|:--|:--|:--|:--|
| 1 🪞 | 거울 | 마담 보바리 | 원하니까 된다고 믿음 | 장부를 펴라 | 문화·컨텐츠 | Dashboard |
| 2 🔭 | 망원경 | 표범 | 다 보이니 원할 이유 없음 | 순서를 타라 | 제조·교통·국방 | D vs S Debate |
| 3 💒 | 스테인드글라스 | 페스트 | 확신이 증거를 차단 | 확신을 교정하라 | 의약·바이오 | Calibration |
| 4 ⏱️ | 시계 | 방드르디 | 성공했으니 바꿀 이유 없음 | 다음 불을 피워라 | AI·에너지 | 2×2 빈칸 |


## 디렉토리

```
chapters/       4장 본문 (경험→이론→사례→실천)
appendices/     부록 (프롬프팅 예시, 산업별 빚 청산)
interactive/    인터랙티브 (주기율표, 감정궤적, 본문뷰어)
docs/           LEXICON, CHANGELOG, INTENT
img/            다이어그램
```

## 관련 레포

- [golden-cage-thesis](https://github.com/hyunjimoon/golden-cage-thesis) — 석사 논문 (N=168,011)

---

*빌린 것을 갚으면 새장이 둥지가 된다.*
